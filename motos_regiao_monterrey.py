...
def get_mottu_token(username: str, password: str) -> str:
    """
    Obtém um novo token JWT da Mottu via SSO.
    Usado para renovar o token automaticamente quando expira.
    """
    ...
    try:
        ...
        if "access_token" in data:
            print(f"[✔] Token Mottu renovado com sucesso (expira em {data.get('expires_in', '?')}s)")
            return data["access_token"]
        else:
            raise ValueError(f"Erro ao obter token: {data}")
    except Exception as e:
        raise RuntimeError(f"Falha na autenticação Mottu: {e}")
 
 
# ================== CONFIGURAÇÕES ==================
 
MOTTU_USERNAME = os.environ.get("MOTTU_USERNAME", "")
MOTTU_PASSWORD = os.environ.get("MOTTU_PASSWORD", "")
TOKEN_MOTTU    = os.environ.get("TOKEN_MOTTU", "")
...
# ── 0. Autenticação — renova token automaticamente via usuário e senha ───────
if MOTTU_USERNAME and MOTTU_PASSWORD:
    print("\nRenovando token Mottu automaticamente...")
    TOKEN_MOTTU = get_mottu_token(MOTTU_USERNAME, MOTTU_PASSWORD)
elif not TOKEN_MOTTU:
    raise RuntimeError("Credenciais não configuradas. Defina MOTTU_USERNAME e MOTTU_PASSWORD nos Secrets.")
...
for i, plate in enumerate(plates, start=1):
    ...
    for fine in fines:
        ...
        print(f"    Link          : https://asp.monterrey.gob.mx/multasdetransito/?placa={fine.get('placa', plate)}")
